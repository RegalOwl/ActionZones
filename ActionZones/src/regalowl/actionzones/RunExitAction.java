...
public class RunExitAction {
	...
	public void runAction(ActionZones actz, Action actio, String zn, Player player) {
		...
		String action = az.getYaml().getZones().getString(zonename + ".exit-action").toLowerCase();		
		actionid = -1;	
		if (actions.containsKey(action)) {			
			actionid = actions.get(action);	
    		long delay = 0L;
    		String testdelay = az.getYaml().getZones().getString(zonename + ".exit-delay");
    		if (testdelay != null) {
    			delay = az.getYaml().getZones().getLong(zonename + ".exit-delay");
    		}
